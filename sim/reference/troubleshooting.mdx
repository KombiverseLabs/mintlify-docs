...
# View Simulate logs
docker logs kombifysim --tail 100
...
1. **Docker socket not mounted**
   ...
   ```bash
   docker inspect kombifysim | grep -A5 Mounts
   ```
...
```bash
# Restart Simulate
docker compose restart kombifysim
...
**Check logs:**
```bash
docker logs kombifysim --tail 200 2>&1 | grep -i error
```
...
1. **Corrupted state**
   ...
   # As a last resort, reset state (loses all simulations)
   docker compose down
   rm -rf ./data/*
   docker compose up -d
   ```
...
2. **Docker API issues**
   ...
   docker exec kombifysim curl --unix-socket /var/run/docker.sock http://localhost/version
   ```

3. **Memory exhaustion**
   ```bash
   docker stats kombifysim --no-stream
...
**Check the service** (the dashboard is served by the same container as the API, on port 5270):
```bash
curl http://127.0.0.1:5270/
docker logs kombifysim --tail 100
```
...
<Tabs>
  ...
  <Tab title="QEMU">
    ...
    **QEMU backend not responding:**
    ...
    echo $KOMBISIM_QEMU_BACKEND_URL

    # Check backend health at that URL
    curl "$KOMBISIM_QEMU_BACKEND_URL/health"
    ```
...
```bash
# Find all errors
docker logs kombifysim 2>&1 | grep -i error

# Find node lifecycle events
docker logs kombifysim 2>&1 | grep -E "node|container"

# Find API requests
docker logs kombifysim 2>&1 | grep -E "POST|GET|DELETE"
```
...
1. **Gather diagnostics:**
   ```bash
   docker logs kombifysim > sim-logs.txt 2>&1
   curl http://127.0.0.1:5270/health > health.json
   docker ps -a > containers.txt
   ```

2. **Check the community page:**
   [Community](/community) — support channels and project links
