...
| Field | Value |
|-------|-------|
| **User** | Shown in the node's connection details |
| **Password** | Generated per node at creation — shown during provisioning |
| **Port Range** | 30000-39999 |

<Note>
  There are no shared default passwords. Each node gets its own generated SSH credentials. Retrieve them anytime via the dashboard or `GET /nodes/{id}/ssh`.
</Note>
...
# Password: <your-node-password> — generated per node, shown during provisioning
```
...
ssh -p 30001 -L 8080:127.0.0.1:80 root@localhost
```

Now access `http://127.0.0.1:8080` to reach the node's web server.
