...
Full virtual machine simulation using QEMU/KVM. Provides hardware-level fidelity with dedicated kernels. QEMU support is experimental and still maturing.
...
| **QEMU** (experimental) | Very High | Slow | Free | Hardware-level testing |
| **Azure** | Production | Medium | Paid | Cloud validation |
| **DigitalOcean** | Production | Medium | Paid | Cloud validation |

On self-hosted installs, set the engine via the `KOMBISIM_ENGINE` environment variable or API parameter. Cloud engines are configured through the dashboard with provider credentials.
