...
<Accordion title="🔧 Technical details" icon="code">
  ...
  **Architecture:** Go backend with REST API for automation. Supports multiple engines for different fidelity levels.
</Accordion>
...
<Note>
  The self-hosted Simulate distribution is not yet published — self-hosting is coming soon. The steps below describe the intended local flow. See [Self-hosting Simulate](/sim/how-to/self-hosting) for status.
</Note>
...
<Steps>
  ...
  <Step title="Create Simulation">
    ```bash
    curl -X POST http://127.0.0.1:5270/api/v1/simulations \
  ...
  <Step title="Add Nodes">
    ```bash
    curl -X POST http://127.0.0.1:5270/api/v1/nodes \
      -H "Content-Type: application/json" \
      -d '{
        "name": "server-1",
  ...
  <Step title="SSH into Node">
    ...
    # Credentials are generated per node and shown in the node's connection details
    ```
  </Step>
</Steps>
...
| **QEMU** (experimental) | Virtual machine | Hardware-level fidelity |
| **Azure** | Cloud VM | Real cloud infrastructure testing |
| **DigitalOcean** | Cloud VM | Real cloud infrastructure testing |

On self-hosted installs, set the engine via the `KOMBISIM_ENGINE` environment variable. Cloud engines are configured through the dashboard with provider credentials.
...
<Note>
  **Base URL:** `https://api.kombify.io/v1/simulation` (SaaS) or `http://127.0.0.1:5270/api/v1` (self-hosted/local)
</Note>
