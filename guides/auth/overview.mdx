...
<Note>
  Interactive sign-in always happens through the hosted **Auth0 Universal Login** page — kombify applications redirect you there and never collect credentials in embedded forms. In the browser and in tokens the issuer is `login.kombify.io`; the underlying tenant domain `kombify.eu.auth0.com` may appear in backend configurations.
</Note>
