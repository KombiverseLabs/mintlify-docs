...
| Adapter | Catalog maturity | Deployment | Route and TLS evidence | Backup and restore |
| --- | --- | --- | --- | --- |
| `standalone-compose` | supported | yes | yes | yes |
| `coolify` | supported | yes | yes | no |
| `komodo` | beta | yes | yes | no |

The generated [application delivery compatibility](/stackkits/reference/application-delivery-compatibility) page owns the complete matrix for the published release, including all workload contracts it covers; consult it rather than assuming this summary is exhaustive. It is catalog compatibility, not proof that your host is healthy.
