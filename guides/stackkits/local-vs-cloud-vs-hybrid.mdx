...
| Hybrid         | You want local data plus cloud reachability or remote workers.                    | Modern Homelab (Preview)  |
...
- `Cloud server` resolves to the Cloud Kit — the same single-environment model on a VPS.
- `Hybrid` increases the fit for Modern Homelab (Preview).
- `Not sure` avoids overfitting and keeps the Basement Kit as the safest first step.
