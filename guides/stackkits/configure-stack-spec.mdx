...
The released CLI writes a complete native StackSpec and establishes local owner custody outside it. Keep the generated `apiVersion`: the installed release's CUE contracts supply its fields, defaults and constraints.
...
The Basement and Cloud Kit contracts expose `files` as an optional workload for the [Documents & Files](/guides/stackkits/use-cases/documents-and-files) use case; the [use-case catalog](/stackkits/reference/use-case-catalog) lists the components declared by the published release. This selection was validated and generated with the released CLI:
