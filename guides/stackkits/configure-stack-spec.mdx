...
### Accept the catalog defaults at init

Since v0.24.63, `stackkit init <kit> --catalog-defaults` materializes the release's CUE catalog defaults into explicit native `v2alpha2` intent:

```bash
stackkit init basement-kit --catalog-defaults --owner-source=local
```

The flag persists the declared default alternatives and compute profiles for omitted choices, so the resulting StackSpec records what the catalog resolved instead of leaving those fields empty. Individual explicit selections such as `--use-case-alternative` take precedence over the materialized defaults. The flag cannot be combined with a persisted candidate spec and does not migrate an existing installation. An omitted adapter choice resolves to `standalone-compose`.
