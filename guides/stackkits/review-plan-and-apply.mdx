...
Review the local approval prompt. `apply` is the step that changes the host.
The toolchain installer leaves that step to you; the Basement bootstrap at
`base.stackkit.cc` already performs the initial apply. After that bootstrap,
apply again only for an intentional configuration change.
