...
<Info>
  Evidence state (2026-09-17, v0.35.0): the Files workload with Cloudreve is
  ...
  the guest-local loopback. Vault (Vaultwarden) passed the same lifecycle with
  its own setup on v0.35.0, and restore activation completed; its
  application-level readback after restore is pending. Photos (Immich) was
  last verified through restore activation on v0.33.3, and its
  application-level readback after restore is pending. Phone, second-device,
  and LAN acceptance are pending.
</Info>
