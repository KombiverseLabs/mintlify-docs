...
StackKits v0.24.2 adds `stackkit setup files`, which verifies the Cloudreve administrator against the applied workload using a private `.stackkit/setup/files-owner.json` credential file. The action uses an existing administrator login unless the credential file explicitly permits first-owner registration. Keep the file private with `chmod 600` and pass `--owner-approve`. See [Set up the application owner](/stackkits/reference/day-2-operations#set-up-the-application-owner).
