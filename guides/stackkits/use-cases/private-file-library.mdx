...
<Info>
  ...
  over the guest-local loopback. On the public v0.31.5 archive, the same
  lifecycle through isolated restore and restore activation passed for Photos
  (Immich) and Vault (Vaultwarden); their application-level readback after
  restore is pending. Phone, second-device, and LAN acceptance are pending.
</Info>
