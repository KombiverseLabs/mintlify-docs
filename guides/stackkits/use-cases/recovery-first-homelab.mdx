...
<Info>
  Evidence state (2026-09-14, v0.31.5): on the public v0.31.5 archive, the
  Basement Kit backup, isolated restore, and restore activation cycle passed on
  Ubuntu 24.04 amd64 for Photos (Immich) and Vault (Vaultwarden); their
  application-level readback after restore is pending. Vault's owner setup
  prepares the invitation only; the official client completes the account. For
  the Files workload (Cloudreve), the byte-level proof stands on the public
  v0.31.1 archive: the restored bytes match the original, activation completed,
  and the application readback was verified over the guest-local loopback.
