---
title: "Personal AI"
description: "Run a private chat workbench on your host with owner custody and explicit model downloads"
sidebarTitle: "Personal AI"
icon: brain
---

<Info>
  Evidence state (2026-09-24, v0.41.0): Personal AI is `available, not yet
  lifecycle-verified`. The current release declares its components (Ollama
  and Open WebUI), and no automated lifecycle run covers install, apply,
  verify, backup, or restore yet. Runtime evidence is pending.
</Info>
...
Open WebUI is browser-based: open your generated AI route in any modern browser and sign in with the administrator account you set. No client installation is required.

<Card title="Ollama" icon="terminal" href="https://ollama.com">
  Ollama itself is a CLI/API model runner rather than an end-user app. StackKits already runs it for you; the upstream site documents the model catalog and CLI if you want to inspect it directly on the host.
</Card>
