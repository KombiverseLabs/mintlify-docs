...
<Info>
There is exactly **one Kombify account** for every Kombify-hosted app and tool.
Auth0 Universal Login at `login.kombify.io` is the only production login
provider. Opening another Kombify product never requires another registration
or product-specific account.
</Info>
