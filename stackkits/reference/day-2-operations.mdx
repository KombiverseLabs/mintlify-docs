...
StackKits v0.21.11 adds `stackkit secrets materialize` as the explicit step between selecting a workload and running `generate`. The command validates the current StackSpec against the embedded CUE authority, then creates or reuses owner-signed, owner-only custody for every `secret://` reference the StackSpec declares. It is idempotent, so re-running it reuses the custody that already exists.

The command never prints secret locators or material. It refuses tampered or foreign custody instead of replacing it. `generate` remains deterministic and `apply` remains read-only toward secret custody; neither mints keys silently. If the StackSpec declares no local secret references, the command reports that and exits successfully.
...
Pass `--terminal-evidence-json` to emit `stackkit.workload-removal-evidence/v1` on stdout:

```bash
stackkit remove --workload photos --auto-approve --terminal-evidence-json
```

The evidence record is a bounded, canonical projection of the exact workload, placement, execution channel, owner authorization, and terminal result. It omits applied artifact content, and `--json` keeps its existing result contract. Pinned cross-repo consumers can validate removal requests and evidence with the public `pkg/workloadremoval` Go package instead of maintaining a second implementation. The evidence digest is an integrity binding, not a new trust root: remote consumers still authenticate the pinned StackKits producer channel and the retained owner authorization.
