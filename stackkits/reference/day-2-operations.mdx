...
These lifecycle checks are the supported way to assess the stack itself. StackKits keeps the observation and verification evidence locally and needs no hosted monitoring service.

The released [Uptime Kuma](/stackkits/apps/uptime-kuma) module is not selected by the initial native v2 StackSpec, so a fresh Basement or Cloud Kit has no generated monitoring address. If you run a separate monitor, add checks only for routes you are authorized to monitor. Protected routes need an intentional health endpoint or an expected authentication response; weakening route protection to obtain HTTP 200 is not a valid monitor setup.
