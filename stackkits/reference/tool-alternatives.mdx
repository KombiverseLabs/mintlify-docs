...
StackKits never locks you into a single tool. For each capability there is a **smart default** (what you get if you change nothing) and **alternatives** you can select in `kombination.yaml`. Defaults are resolved from your context (local vs cloud), compute tier, and domain — you can always override them.
...
<Steps>
  <Step title="Explicit override">
    Anything you set in `kombination.yaml` wins.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Spec format" icon="file-code" href="/stackkits/reference/spec-format">
    Full `kombination.yaml` reference.
  </Card>
</CardGroup>
