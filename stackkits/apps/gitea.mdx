...
The published release archive contains the complete example selection at `examples/v2/gitea-standalone.stack-spec.yaml`. It selects the optional `dev` workload with the `gitea` alternative on the `standalone-compose` adapter, binds private `personal`-class data, and declares an HTTPS route (`host`, `port 443`, target port `3000`) plus an internal-PKI capability. Adapt the node identity, home domain, and host storage roots to your host — Gitea generation requires that explicit HTTPS route and data binding so native clients receive a usable governed clone URL.

From a fresh workspace, preserve those selections with a candidate spec:

```bash
stackkit init basement-kit --candidate-spec ./gitea-standalone.stack-spec.yaml --owner-source local --owner-email owner@example.com --non-interactive
