...
<Tabs>
  <Tab title="Baseline">
    ...
    | **Coolify** | App platform (PaaS) — smart default; Dokploy/Komodo selectable |
...
<Note>
  The app platform is **context-resolved** — the Base Kit does not hard-pin a PaaS. On standard/low tiers the resolver enables **Coolify** by default; the high tier also adds Portainer, Prometheus, and Grafana. Override any choice in your spec — see [tool alternatives](/stackkits/reference/tool-alternatives) for the full matrix.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Coolify" icon="cloud">
    **App platform (smart default)**
    
    Self-hosted PaaS like Vercel/Heroku. Deploy from Git, manage databases, scale apps. Enabled by default — swap for Dokploy or Komodo with one override.
  </Card>
  <Card title="Dokploy / Komodo" icon="rocket">
    **Alternative app platforms**
    
    Prefer a different PaaS? Select Dokploy (Vercel/Railway-style) or Komodo instead of Coolify.
  </Card>
