...
Use it when a single-environment [Basement Kit](/stackkits/kits/basement-kit) does not fit — you need public-facing services on a VPS, applications and data staying at home, and the two halves governed as one stack.

<Note>
  Modern Homelab became functional in StackKits **v0.9.0** and now sits on the same standalone lifecycle as the Basement Kit. If you only need a single-node baseline first, start with the [StackKits quickstart](/stackkits/quickstart) and the [Basement Kit](/stackkits/kits/basement-kit).
</Note>
...
- **Governed warm-standby placement, no separate high-availability kit.** Warm-standby placement for critical services is a governed feature of Modern Homelab itself. There is no separate high-availability kit to install to get resilient placement across the two sites.
...
<Warning>
  The Modern Homelab Kit requires both cloud and local infrastructure. If you only need a single environment (local or cloud), use the [Basement Kit](/stackkits/kits/basement-kit) instead.
</Warning>
...
Coolify manages deployments across your nodes. While the Basement Kit defaults to Coolify for single-environment setups, the Modern Homelab Kit leverages its multi-node capabilities for hybrid topologies:
...
| Feature | Basement Kit | HA add-on | Modern Homelab Kit |
|---------|-------------|------------|----------------|
...
## Migration from the Basement Kit
