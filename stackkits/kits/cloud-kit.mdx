...
|---------|------------------------------|----------------------------|
| TLS | Self-signed on a local domain you pick (for example `home.test`) | Let's Encrypt (ACME) on a real domain |
| Access | Host ports, private IP | Reverse proxy, public IP |
| Domain | Required — any resolvable `network.domain.base` (local or routable) | Required (own/custom domain or `kombify.me`) |
