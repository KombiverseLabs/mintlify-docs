...
kombify Cloud uses Auth0 for hosted SSO. All interactive sign-in happens through the hosted **Universal Login** page — kombify never collects credentials in embedded forms.

<Note>
  **Two-domain model:** in the browser and in issued tokens, the issuer is `login.kombify.io`. The underlying Auth0 tenant domain `kombify.eu.auth0.com` may still appear in backend configuration values — both refer to the same tenant.
</Note>
